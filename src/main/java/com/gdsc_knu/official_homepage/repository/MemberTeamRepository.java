...
public interface MemberTeamRepository extends JpaRepository<MemberTeam, Long> {
    ...
    @Query("SELECT new com.gdsc_knu.official_homepage.dto.admin.team.AdminTeamMemberResponse(m.id, m.name, m.email, m.profileUrl) " +
            "FROM MemberTeam mt " +
            "JOIN mt.member m " +
            "WHERE mt.team.id = :teamId")
    List<AdminTeamMemberResponse> findAllAdminMemberResponsesByTeamId(@Param("teamId") Long teamId);
